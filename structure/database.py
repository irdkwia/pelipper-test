...
class Connection:
    ...
    def update_elements(self, elist, conditions=None):
        ...
        for element in elist:
            ...
            query = "UPDATE %s SET %s" % (
                self.unifiedname(element.__class__.__name__),
                ",".join("%s=:%s" % (k, k) for k in d),
            )
    ...
    def increment_count(self, elist, counters, conditions=None):
        ...
        for element in elist:
            ...
            query = "UPDATE %s SET %s" % (
                self.unifiedname(element.__class__.__name__),
                ",".join("%s=%s+1" % (k, k) for k in counters),
            )
            if conditions is None:
                conditions = dict()
            query += " WHERE %s" % (
    ...
    def get_elements(
        self, cls, conditions=None, ordering=None, limit=None, include=None, mutual=None
    ):
        con = self.connect()
        cur = con.cursor()
        ...
        if mutual is not None:
            ...
            pf = "l."
            query += " AND ".join(
                "l.%s=r.%s AND l.%s=r.%s" % (m[0], m[1], m[1], m[0]) for m in mutual
            )
        else:
        ...
        if include is not None:
            totalcond.extend(
                pf + "%s IN (%s)" % (k, ",".join([str(x) for x in v]))
