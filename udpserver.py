...
from socketserver import DatagramRequestHandler, ThreadingMixIn, UDPServer
...
class CustomHandler(DatagramRequestHandler):
    def handle(self):
        ...
        if mtype == 9:
            ...
            self.wfile.write(cid.to_bytes(4, "big"))
            self.wfile.write(
                "".join([choice(TOKENPOOL) for i in range(6)]).encode("ascii")
            )
            self.wfile.write(b"00")
            localip = hexlify(
                bytes([int(x) for x in struct_info["localip0"].split(".")])
            )
