...
class CustomHandler(BaseRequestHandler):
    def challenge(self, sschal, sstoken, mmchal, yychal):
        return md5(
            md5(sschal.encode("ascii")).hexdigest().encode("ascii")
            + b" " * 48
            + sstoken.encode("ascii")
            + mmchal.encode("ascii")
            + yychal.encode("ascii")
            + md5(sschal.encode("ascii")).hexdigest().encode("ascii")
        ).hexdigest()
    ...
    def parsemessage(self):
        ...
        while f != "final":
            ...
            f = self.getfield()
        if action == "login":
            self.profile = self.db.get_elements(
                GlobalProfile, {"token": data["authtoken"]}, limit=1
            )[0]
            # print("Log in")
            assert data["response"] == self.challenge(
                self.profile._challenge,
                data["authtoken"],
                data["challenge"],
                self.mmchal,
            )
            buddylist = [
                str(b.buddy)
                for b in self.db.get_elements(
                    BuddyList, {"pid": self.profile.profileid}
                )
            ]
            self.sendmessage(
                [
                    ...
                    ("uniquenick", self.profile.uniquenick),
                    ("lt", "1"),
                    ("id", data["id"]),
                ]
            )
            ...
            if pid == self.profile.profileid:
                ...
                profile = self.db.get_elements(
                    GlobalProfile, {"profileid": pid}, limit=1
                )[0]
            self.sendmessage(
                [("pi", "2")]
            ...
            if (
                len(
                    self.db.get_elements(
                        BuddyList,
                        {
                            "pid": self.profile.profileid,
                            "buddy": int(data["newprofileid"]),
                        },
                        limit=1,
                    )
                )
                ...
                len(
                    self.db.get_elements(
                        Profile, {"pid": int(data["newprofileid"])}, limit=1
                    )
                )
                == 0
            ):
                self.sendmessage(
                    [
                        ("error", ""),
                        ("err", 1539),
                        ("errmsg", "The profile requested does not exist."),
                    ]
                )
            else:
                pass
                ...
                self.sendmessage(
                    [("bm", "2"), ("f", data["newprofileid"]), ("msg", msg)]
                )
        elif action == "delbuddy":
            self.db.delete_elements(
                BuddyList,
                {"pid": self.profile.profileid, "buddy": int(data["delprofileid"])},
            )
        elif action == "authadd":
            # print("Auth Add")
            pass
            ...
            for pid in data["opids"].split("|"):
                profile = self.db.get_elements(GlobalProfile, {"profileid": int(pid)})[
                    0
                ]
