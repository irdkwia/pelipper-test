...
from handlers.notifhandler import NotifHandler
        ...
        if sid != -1 and psplit.path[sid:].startswith("/web"):
            ...
            if "hash" in query:
                ...
                if new_path in ["/team/teamExist.asp", "/team/teamEntry.asp"]:
                    ...
                    try:
                        t = threading.Thread(
                            target=NotifHandler.send_rescue_notifications,
                            args=[db, prf, rq],
                        )
                        t.start()
                    except Exception:
                    ...
                    if len(rq) > 0:
                        ...
                        if db.update_elements([rq], {"completed": 0}):
                            ...
                            try:
                                t = threading.Thread(
                                    target=NotifHandler.send_aok_notifications,
                                    args=[db, prf, rq, aok],
                                )
                                t.start()
                            except Exception:
                    ...
                    if len(aok) > 0:
                        ...
                        thk.message = translate(
                            data[40:112].decode("utf-16-be").replace("\x00", ""),
                            prf.lang,
                        )
                        ...
                        try:
                            t = threading.Thread(
                                target=NotifHandler.send_thankyou_notifications,
                                args=[db, prf, aok, thk],
                            )
                            t.start()
                        except Exception:
