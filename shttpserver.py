...
cert = b64decode(
    cert.split("-----BEGIN CERTIFICATE-----")[1].split("-----END CERTIFICATE-----")[0]
)
master = b64decode(
    master.split("-----BEGIN CERTIFICATE-----")[1].split("-----END CERTIFICATE-----")[0]
)
...
class CustomHandler(BaseRequestHandler):
    ...
    def sendfinished(self, currentmsg):
        md5_hash = md5(
            ...
            + b"\\" * 48
            + md5(
                bytes(self.hchkall + currentmsg) + b"SRVR" + self.master + b"6" * 48
            ).digest()
        ).digest()
        sha1_hash = sha1(
            self.master
            + b"\\" * 40
            + sha1(
                bytes(self.hchkall + currentmsg) + b"SRVR" + self.master + b"6" * 40
            ).digest()
        ).digest()
    ...
    def parsemessage(self):
        ...
        if msgtype == 0x00:
            ...
            if handshaketype == 1:
                ...
                suites = parse(
                    data[39 + ssid_len + 2 : 39 + ssid_len + 2 + suites_len], 2
                )
                ...
                if 0 not in methods or 5 not in suites or sslv != 0x300:
                    if SERVER_DEBUG:
                        print(getident() + "Start common HTTPS communication")
                    ...
                    while True:
                        ...
                        time.sleep(0.001)
                    if SERVER_DEBUG:
                        print(getident() + "End common HTTPS communication")
                    return False
                else:
                    self.underlying = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                ...
                md5_hash = md5(
                    ...
                    + b"\\" * 48
                    + md5(
                        bytes(self.hchkall) + b"CLNT" + self.master + b"6" * 48
                    ).digest()
                ).digest()
                sha1_hash = sha1(
                    self.master
                    + b"\\" * 40
                    + sha1(
                        bytes(self.hchkall) + b"CLNT" + self.master + b"6" * 40
                    ).digest()
                ).digest()
            ...
            if SERVER_DEBUG:
                print(getident() + "Start DS HTTP communication")
            ...
            if SERVER_DEBUG:
                print(getident() + "End DS HTTP communication")
            return False
        else:
    ...
    def handle(self):
        ...
        try:
            ...
            traceback.print_exc()
        if SERVER_DEBUG:
            print(getident() + "Close underlying")
